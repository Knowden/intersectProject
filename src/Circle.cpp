...
vector<Point> Circle::calculatePointsAtX(double x) {
   ...
   const double common_part = sqrt(pow(r, 2) - pow(x - center->x, 2));
   const double y1 = center->y + common_part;
   const double y2 = center->y - common_part;
...
vector<Point> Circle::calculateIntersectionWithNormalLine(Line& line) {
    ...
    const double t = line.b - center->y;

    const double a = pow(line.k, 2) + 1;
    const double b = 2 * (line.k * t - center->x);
    const double c = pow(t, 2) + pow(center->x, 2) - pow(r, 2);

    const double delta = sqrt(b * b - 4 * a * c);

    const double x1 = (-b + delta) / (2 * a);
    const double x2 = (-b - delta) / (2 * a);
